...
class Ske_MixF(nn.Module):
    def __init__(self, in_channels, out_channels, A, Frames, stride=1, residual=True):
        ...
        # Only keep spatial attention, completely remove temporal processing
        self.spa_mixf = Spatial_MixFormer(in_channels, out_channels, A)
        
        # If stride is needed, only handle it in residual connection
        self.stride = stride
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        if self.stride > 1:
            x_spa = F.avg_pool2d(x_spa, kernel_size=(self.stride, 1), stride=(self.stride, 1))
        
        return self.relu(x_spa + self.residual(x))
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3):
        ...
        # Ultra-lightweight network layers - removed all temporal convolutions
