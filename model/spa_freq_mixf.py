...
class Spatial_Freq_MixFormer(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups=8, num_subset=3,t_stride=1,t_padding=0,t_dilation=1,bias=True,first=False,residual=True):
        super(Spatial_Freq_MixFormer, self).__init__()
        self.groups=groups
        self.out_channels=out_channels
    ...
    def forward(self, x0):
        ...
        for i in range(self.num_subset):  
            ...
            m1d, Q1d, K1d = self.Spa_Att(dct.dct(m[:,i,:(kc// self.num_subset)//4,:,:]))
            m2d, Q2d, K2d = self.Spa_Att(dct.dct(m[:,i,(kc// self.num_subset)//4:((kc// self.num_subset)//4)*2,:,:]))
            m3d, Q3d, K3d = self.Spa_Att(dct.dct(m[:,i,((kc// self.num_subset)//4)*2:((kc// self.num_subset)//4)*3,:,:]))
            m4d, Q4d, K4d = self.Spa_Att(dct.dct(m[:,i,((kc// self.num_subset)//4)*3:((kc// self.num_subset)//4)*4,:,:]))
            

            m1_2d = self.soft(torch.einsum('nv,nw->nvw', (self.relu(self.linear(K1d.squeeze(-1).squeeze(-1))), self.relu(self.linear(Q2d.squeeze(-1).squeeze(-1)))))).unsqueeze(1).repeat(1,self.out_channels//4,1,1)
            m2_3d = self.soft(torch.einsum('nv,nw->nvw', (self.relu(self.linear(K2d.squeeze(-1).squeeze(-1))), self.relu(self.linear(Q3d.squeeze(-1).squeeze(-1)))))).unsqueeze(1).repeat(1,self.out_channels//4,1,1)
            m3_4d = self.soft(torch.einsum('nv,nw->nvw', (self.relu(self.linear(K3d.squeeze(-1).squeeze(-1))), self.relu(self.linear(Q4d.squeeze(-1).squeeze(-1)))))).unsqueeze(1).repeat(1,self.out_channels//4,1,1)
            ...
            m1d = dct.idct(m1d/2 + m1_2d/4)
            m2d = dct.idct(m2d/2 + m1_2d/4 + m2_3d/4)
            m3d = dct.idct(m3d/2 + m2_3d/4 + m3_4d/4)
            m4d = dct.idct(m4d/2 + m3_4d/4)
